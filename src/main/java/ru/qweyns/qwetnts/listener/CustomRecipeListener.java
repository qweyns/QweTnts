...
public final class CustomRecipeListener implements Listener {
    ...
    public void onPrepare(@NotNull PrepareItemCraftEvent event) {
        ...
        try {
            ...
            for (Map.Entry<Integer, String> entry : requirements.entrySet()) {
                ...
                String found = stack.getItemMeta().getPersistentDataContainer().get(
                        requirementKey(kind), PersistentDataType.STRING);
                String expected = stripPrefix(kind);
                if (found == null || !found.equalsIgnoreCase(expected)) {
                    inventory.setResult(null);
                    return;
                }
            }
    ...
    /**
     * Какой PDC-ключ проверять у ингредиента.
     *
     * <p>Требование может быть двух видов: {@code tnt:<id>} — нужен конкретный
     * динамит, {@code component:<id>} — конкретный компонент крафта (например,
     * {@code component:explosive} — Взрывчатое вещество из HolyWorld).</p>
     */
    private @NotNull NamespacedKey requirementKey(@NotNull String requirement) {
        return requirement.startsWith(ComponentType.PREFIX)
                ? plugin.keys().component
                : plugin.keys().dynamiteKind;
    }

    /** Значение метки без префикса вида {@code component:}. */
    private static @NotNull String stripPrefix(@NotNull String requirement) {
        return ComponentType.stripPrefix(requirement);
    }
}
