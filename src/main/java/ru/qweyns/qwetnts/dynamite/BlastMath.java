...
public final class BlastMath {
    ...
    /**
     * Сработал ли дробный шанс в процентах (0,4 % — не то же, что 0 % или 1 %).
     *
     * <p>Нужно механике бункера: на HolyWorld шанс пробить стену обычным
     * динамитом равен 0,4 %, и округлять его до нуля нельзя — иначе заряд
     * перестал бы работать вовсе.</p>
     */
    public static boolean rollPercent(double chancePercent, @NotNull RandomGenerator random) {
        if (!(chancePercent > 0.0)) return false;
        if (chancePercent >= 100.0) return true;
