...
| Файл | Назначение |
| :--- | :--- |
| `config.yml` | Язык, автоподжог, анти-лаг, рейд-блоки, временные блоки, алерты, bStats |
...
- Базовая цель сборки — Paper API 1.21.4, а не гарантия совместимости со
  всеми будущими 1.21+.
- QPS — обязательная зависимость (`depend` в `plugin.yml`). Без него плагин
  отключается при старте, не падая.
